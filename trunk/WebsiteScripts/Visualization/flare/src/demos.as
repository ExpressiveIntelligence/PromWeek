package {
	...
	public class demos extends Sprite
	{
		...
		{
			trace ("ShowDemo::" + _cur)
			// magic number here O_O wtf guys
			if (_cur >= 0) {
				_demos[_cur].stop();
			...
			// used to get the number of 'which guy is selected' from the array _links (the array of demo elements along the top)
			var tgt:DisplayObject = event.target as DisplayObject;
			// get the index number of it
			_cur = _links.getChildIndex(tgt);
			// make it transparent? I believe this is used for making the transition pretty. The process 
			// fades in the newly selected one at the same time the current one fades out.
			_demo.alpha = 0;
			// make it a child of the demo window for displaying (as the base Sprite Object)
			_demo.addChild(_demos[_cur] as Sprite);
			
			// create a new transitioner class. Not sure about exactly how to use these yet but what it does is clear.
			// This little snippet here creates a new Transitioner for just the logo (for some reason, just the logo, not
			// for the currently selected demo.
