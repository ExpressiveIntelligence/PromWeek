package  
{
	...
	public class Story 
	{
		...
		*/
		 ...
		 * @return	True if the goals were satisfied either conjunctively or disjunctively.
		 */
		public function evaluateToDoItems(storyLeadCharacter:String, cast:Vector.<Character>, isConjunction:Boolean = false):Vector.<ToDoItem>
		{
			var isGoalSatisfied:Boolean = false;
			var rule:Rule;
			var firstChar:Character;
			...
			if (isConjunction) {
				...
				//for each(firstChar in cast) {
				firstChar = CiFSingleton.getInstance().cast.getCharByName(storyLeadCharacter);
				for each(secondChar in cast) 
				{
					if (firstChar.characterName.toLowerCase() != secondChar.characterName.toLowerCase()) 
					{
